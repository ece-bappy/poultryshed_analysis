...
for day, group in grouped_controlled:
    plt.plot(group['Time'], group['Temperature'], label=f'Day {day} - Controlled Environment', color='red')

# Plot temperature for traditional environment
for day, group in grouped_traditional:
    plt.plot(group['Time'], group['Temperature'], label=f'Day {day} - Traditional Environment', color='blue')

plt.title('Temperature Comparison between Controlled(Red) and Traditional Environments(Blue)')
plt.xlabel('Time')
